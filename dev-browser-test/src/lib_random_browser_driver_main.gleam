...
const module_path = "/symbolize_lib_random/lib_random_browser_test_main.mjs"

pub fn main() {
  let root = browser.resolve_path("../lib-random/build/dev/javascript")
  browser.run(
    ...
    [] ->
      case
        browser.report(page, server, [
          "lib-random Chromium Web Crypto/Math FFI passed",
        ])
      {
        Error(reason) -> Error(reason)
        Ok(Nil) -> {
          io.println("lib-random Chromium Gleam driver passed")
          Ok(Nil)
        }
      }
