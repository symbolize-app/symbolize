...
readJSON :: (Aeson.FromJSON a, Eq a, Show a, Typeable a) => FilePath -> a -> Result
readJSON = _result1 (Command.ReadJSON FileFormat.JSON)

writeJSON :: (Aeson.ToJSON b, Eq b, Show b, Typeable b) => FilePath -> b -> Result
writeJSON = _result2' (Command.WriteJSON FileFormat.JSON)

readYAML :: (Aeson.FromJSON a, Eq a, Show a, Typeable a) => FilePath -> a -> Result
readYAML = _result1 (Command.ReadJSON FileFormat.YAML)

writeYAML :: (Aeson.ToJSON b, Eq b, Show b, Typeable b) => FilePath -> b -> Result
...
_result1 :: (Typeable a, Eq a, Show a) => (t1 -> Command.Command a) -> t1 -> a -> Result
