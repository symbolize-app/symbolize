...
export interface ExpressionIntern {
  ...
  extraRules(): IterableIterator<stylingRule.Rule>
  toPure(ctx: stylingContext.Context): PureExpressionIntern
}
...
export function compilePure(
  ...
  const mutableExtraRules: stylingRule.Rule[] = []
...
class PureExpressionIntern implements ExpressionIntern {
  ...
  constructor(
    ...
    private readonly extraRulesArray: readonly stylingRule.Rule[],
  ) {}
...
function extractCustomProperty(
  ...
  const className = ctx.styling.expressionClassName.build()
  const propertyName = ctx.styling.expressionCustomPropertyName.build()
  const code = expressionIntern.code(propertyName)
  const pseudoElement = null
  const rule = stylingRule.rule(className, pseudoElement, code)
